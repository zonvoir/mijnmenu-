...
export class AuthComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    checkSubscription() {
      this.loading = true;
      this.model.subscription = this.selectedItem;
      if (this.selectedItem.toLowerCase() == 'bronze') {
        this.signup();
      } else {
        this.display = 'block';
      }
    ...
    forgotPass() {
        ...
        this._userService.forgotPassword(this.model.email).subscribe(
            data => {
                this.showAlert('alertSignin');
                this._alertService.success(
                    'Cool! Password recovery instruction has been sent to your email. ' + this.model.email,
                    true);
                this.loading = false;
                this.displaySignInForm();
                this.model = {};
            ...
            error => {
                var errorMsg;
                if (error._body != null && error._body != undefined && error._body != '') {
    ...
    subscription(stype, amount) {
      this.selectedItem = stype;
      this.amount = amount;
    }
}
